...
class GCSGraphics(QWidget):
    ...
    def update_style(self, palette=None):
        ...
        self.hg = palette.color(QPalette.Active, QPalette.Highlight)
    ...
    def paintEvent(self, e):
        ...
        if self.debug:
            self.draw_inf_marker(self.pitch, self.bank)
            self.draw_inf_marker(0, 0)
        self.draw_markers()
        self.draw_cursor()
        self.draw_skipskid()
        self.draw_heading()
    ...
    def draw_marker(self, p, r, t="", alpha=128):
        ...
        s = self.scale
        if self.debug:
            painter.drawEllipse(w/2 - p*sin(b) - 10*s, h /
                                2*s + p*cos(b) - 10*s, 20*s, 20*s)
    ...
    def draw_inf_marker(self, p, b):
        ...
        h = self.height
        s = self.scale
        x1, y1, x2, y2 = self.compute_horizon(w, h, p, b)
        painter.drawLine(QLineF(x1, y1, x2, y2))
        if self.debug:
            painter.drawEllipse(x1 - 10*s, y1 - 10*s, 20*s, 20*s)
            painter.drawEllipse(x2 - 10*s, y2 - 10*s, 20*s, 20*s)

    def compute_horizon(self, w, h, p, b):
    ...
    def draw_region(self, sky=False):
        ...
        if self.debug:
            for point in points:
                painter.drawEllipse(
                    point.x() - 10*s, point.y() - 10*s, 20*s, 20*s)
